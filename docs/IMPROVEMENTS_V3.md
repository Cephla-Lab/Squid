...
10. [V3 Additions Overview](#10-v3-additions-overview)
11. [Config and Dependency Injection Track](#11-config-and-dependency-injection-track)
12. [HAL/ABC Completion Track](#12-halabc-completion-track)
13. [Decouple squid from control](#13-decouple-squid-from-control)
14. [Runtime Controls: Backpressure, Drop Policy, Metrics](#14-runtime-controls-backpressure-drop-policy-metrics)
15. [Toolkit Consistency for Worker Management](#15-toolkit-consistency-for-worker-management)
16. [Testing and Verification Track](#16-testing-and-verification-track)
17. [Logging and Traceability](#17-logging-and-traceability)
...
## 10. V3 Additions Overview

This version keeps all V2 content above and layers in additional tracks to address the biggest gaps noted in `docs/ARCHITECTURE_V2.md`: global configuration singletons, incomplete HAL/ABC migration, tight squid→control coupling, backpressure policies, Qt-threading consistency, and missing test/logging infrastructure.

---

## 11. Config and Dependency Injection Track

**Problem:** `software/control/_def.py` mutates globals at import; `squid/config.py` snapshots those into Pydantic models once. Multiple instances/sessions cannot coexist; type validation is weak; import order matters.

**Implementation Steps**
1) Create a pure `config_loader` that reads INI, validates with Pydantic, and returns an immutable config object graph; fail fast on bad data.
2) Add `Microscope.build_from_config(config, simulated=False)` and pass config objects into controllers/GUI (no `_def` reads).
3) Keep a thin compatibility shim that builds config objects from `_def` for legacy code, but new code uses injected config.
4) Add tests for parsing/validation with good/bad fixtures.

---

## 12. HAL/ABC Completion Track

**Problem:** Many `software/control/camera_*.py` drivers do not implement `AbstractCamera`; `SimulatedCamera.__getattr__` masks gaps. Similar issues may exist for stages/filter wheels.

**Implementation Steps**
1) Inventory all drivers; wrap non-conforming ones to subclass `AbstractCamera` (and stage/filter ABCs).
2) Remove `__getattr__` shims; hard-fail on missing methods.
3) Update `squid.camera.utils.get_camera` to reject non-ABC implementations and require trigger/strobe callbacks where needed.
4) Add interface tests for simulated hardware to enforce ABC behavior.

---

## 13. Decouple squid from control

**Problem:** `squid/config.py` and `squid/abc.py` import `control` globals, so the HAL is not reusable/testable in isolation.

**Implementation Steps**
1) Move shared pure types/utilities into a small common module (no globals).
2) Remove `_def` imports from `squid/config.py`; consume injected config objects.
3) Ensure `squid/abc.py` has no `control` imports; relocate shared exceptions/helpers to common.
4) Use DI for hardware composition (`MicroscopeAddons`, etc.) instead of reading `_def`.

---

## 14. Runtime Controls: Backpressure, Drop Policy, Metrics

**Problem:** Callback wrappers in V2 catch errors but don’t define overload behavior. Without backpressure, cameras may stall; without metrics, stalls are invisible.

**Implementation Steps**
1) Define per-stream policy: drop for display; block with timeout or fail fast for acquisition.
2) Add bounded queues in `StreamHandler`/`MultiPointWorker`; log drop counts; expose counters (frames received/dropped, latency).
3) Surface metrics via periodic logging or a debug panel (fps, drops, queue depth).
4) Stress-test with simulated cameras that overproduce frames; assert drop/block behavior.

---

## 15. Toolkit Consistency for Worker Management (PyQt5 via qtpy)

**Problem:** Mixing Qt timers with non-Qt executors on the UI thread is risky. Standardize on Qt threading for GUI-adjacent work.

**Implementation Steps**
1) Use `QThreadPool/QRunnable` or `QtConcurrent.run` + `QFutureWatcher` with queued connections for cross-thread UI updates.
2) Host any `QTimer` in the main (GUI) thread; dispatch work to the pool. For non-Qt logic, you may keep `threading`/`multiprocessing`, but route results to the UI via queued signals.
3) If both worlds are kept, split modules: `worker_manager_qt.py` (Qt objects) vs. `worker_manager_std.py` (pure logic, no Qt objects).
4) Ensure timeouts fire in the owning thread and dump stacks via `faulthandler`; keep imports through `qtpy` to ease a future PyQt6 move.

---

## 16. Testing and Verification Track

**Problem:** No dedicated tests for the new robustness features.

**Implementation Steps**
1) Config parsing tests (good/bad fixtures).
2) ABC compliance tests for simulated hardware.
3) Stress tests: simulated acquisition with intentional timeouts/overload; assert graceful abort, drop counts, and metrics.
4) Integration harness: headless `MultiPointWorker` with simulated microscope saving to temp, verifying OME-TIFF metadata.
5) CI: add tests; mark long stress runs as nightly.

---

## 17. Logging and Traceability

**Problem:** Failures are hard to correlate across threads; operation IDs are not propagated.

**Implementation Steps**
1) Structured logging with operation/request IDs (from `OperationTracker`) across callbacks, worker timeouts, and job dispatch.
2) Include stack traces and IDs in `safe_callback` error payloads.
3) On timeout, dump thread traces (`faulthandler.dump_traceback`) with task/operation IDs.
4) Make structured logging opt-in via config to limit overhead.
