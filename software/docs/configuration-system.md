...
| `filter_position` | Emission filter wheel position |
| `enabled` | Whether channel is enabled for selection |
...
| `exposure_time_ms` | Camera exposure time in ms |
| `gain_mode` | Camera analog gain |
| `confocal_override` | Settings for confocal mode (only if confocal present) |
...
| `filter_position` | general | Filter setup |
