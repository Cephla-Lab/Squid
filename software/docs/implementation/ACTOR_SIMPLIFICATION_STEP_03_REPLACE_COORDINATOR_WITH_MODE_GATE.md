...
- [ ] Add `src/squid/core/mode_gate.py` with `GlobalMode` enum (reuse existing names).
- [ ] Add `GlobalModeChanged` event (if not already present post-cleanup).
- [ ] Gate is created in `ApplicationContext` and stored as `self._mode_gate`.
- [ ] Provide `.mode_gate` property for dependency injection.

### 3.2 Remove ResourceCoordinator
- [ ] Delete `src/squid/core/coordinator.py`.
- [ ] Remove all imports/uses of `ResourceCoordinator`, `ResourceLease`, `ACQUISITION_REQUIRED_RESOURCES`, etc.
  - Grep: `rg "ResourceCoordinator|ResourceLease|acquire\\(|release\\(|LEASE" src/squid -S`
- [ ] Remove coordinator build/start/stop from `ApplicationContext`.
...
- [ ] Add optional `mode_gate` dependency to each service ctor OR access via EventBus subscription to `GlobalModeChanged`.
