...
def test_pid_fault_bits_are_parsed_and_logged_once(caplog):
    ...
    def packet(button_and_switch_state, z_cause=0, flags=0):
        ...
        # Reporting off: X / Y causes in byte 19 bits 1-4 / byte 20 bits 0-3, Z's in byte 20 bits 4-7 (byte 19
...
def _fault_cause_packet(crc_calculator, fault_bits=0, x_cause=0, y_cause=0, z_cause=0, flags=None, dev=0, enc=0, z=0):
    ...
    if flags is None:
        ...
        msg[19] = (x_cause & cause.MASK) << cause.X_SHIFT
        msg[20] = ((y_cause & cause.MASK) << cause.Y_SHIFT) | ((z_cause & cause.MASK) << cause.Z_SHIFT)
    else:
        msg[19] = flags
        msg[20:22] = int(dev).to_bytes(2, "big", signed=True)
    msg[22] = (1 << 4) | 6
...
def test_enc_flag_fields_are_not_decoded_from_the_cause_layout():
    ...
    try:
        ...
        # A Y cause (byte 20 in the packed layout) must not leak into the ENC_FLAG fields either.
        _feed_status_packet(
            micro,
            _fault_cause_packet(
                crc_calculator,
