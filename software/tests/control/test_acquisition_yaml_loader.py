...
"""
    ...
    def test_parse_legacy_grid_format(self, tmp_path):
        """Test parsing legacy YAML format with grid section instead of z_stack/time_series."""
        yaml_content = """
acquisition:
  widget_type: wellplate
  xy_mode: Select Wells
...
"""
        ...
        # Values should come from grid section
        assert result.nz == 5
        assert result.nt == 10
        assert result.delta_z_um == 2.0  # 0.002 mm * 1000
        assert result.delta_t_s == 30.0
        # scan_positions should be used for regions
        assert result.scan_size_mm == 2.1
        assert result.overlap_percent == 15.0
    ...
    def test_parse_new_format_takes_precedence(self, tmp_path):
        """Test that new format sections take precedence over legacy grid format."""
        yaml_content = """
acquisition:
  widget_type: wellplate
grid:
...
"""
        ...
        assert result.nz == 10
        assert result.nt == 20
        assert result.delta_z_um == 5.0  # 0.005 mm * 1000
        assert result.delta_t_s == 60.0
        assert result.scan_size_mm == 3.0
        assert result.overlap_percent == 20.0
        assert len(result.wellplate_regions) == 1
        assert result.wellplate_regions[0]["name"] == "D5"
