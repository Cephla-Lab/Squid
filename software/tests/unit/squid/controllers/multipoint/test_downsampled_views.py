...
Ported from upstream commit ad9479d for arch_v2.
"""
...
class TestPlateViewManager:
    ...
    def test_plate_view_manager_init_multi_channel(self):
        ...
        manager = DownsampledViewManager(
            4, 6, (50, 50), num_channels=3, channel_names=["DAPI", "GFP", "RFP"]
        )
    ...
    def test_plate_view_manager_update_well(self):
        ...
        x_start = 2 * 100
        assert np.all(
            manager.plate_view[0, y_start : y_start + 80, x_start : x_start + 80] == 5000
        )
    ...
    def test_plate_view_multi_channel_update(self):
        ...
        manager = DownsampledViewManager(
            2, 2, (50, 50), num_channels=2, channel_names=["Ch0", "Ch1"]
        )
    ...
    def test_format_well_id_inverse_of_parse(self):
        ...
        for row in range(32):
            for col in range(48):
                ...
                assert (parsed_row, parsed_col) == (
                    row,
                    col,
                ), f"Round-trip failed for ({row}, {col}) -> {well_id}"
...
class TestConfigValidation:
    ...
    def test_config_plate_resolution_already_in_list(self):
        ...
        result = ensure_plate_resolution_in_well_resolutions(
            well_resolutions, plate_resolution
        )
