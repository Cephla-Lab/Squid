...
Ported from upstream test_zarr_writer.py with import path remappings
for arch_v2 project structure.
"""
...
from _def import ZarrChunkMode, ZarrCompression
...
def make_test_capture_info(
    ...
    return CaptureInfo(
        position=squid.core.abc.Pos(x_mm=0.0, y_mm=0.0, z_mm=0.0, theta_rad=None),
        z_index=z_index,
...
class TestZarrAcquisitionConfig:
    ...
    def test_config_creation(self):
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig
...
class TestChunkShapeCalculation:
    ...
    def test_full_frame_chunks(self):
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, _get_chunk_shape
    ...
    def test_shard_shape_per_z_level(self):
        ...
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, _get_shard_shape
    ...
    def test_fast_mode_no_sharding(self):
        ...
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, _get_shard_shape, _get_chunk_shape
...
class TestCompressionCodecs:
    ...
    def test_none_compression(self):
        ...
        from squid.backend.io.writers.zarr_writer import _get_compression_codec
...
class TestDtypeConversion:
    ...
    def test_common_dtypes(self):
        from squid.backend.io.writers.zarr_writer import _dtype_to_zarr
...
class TestZarrWriter:
    ...
    def test_sync_lifecycle(self, temp_dir):
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter
...
class TestHCSMetadata:
    ...
    def test_write_plate_metadata(self, temp_dir):
        from squid.backend.io.writers.zarr_writer import write_plate_metadata
    ...
    def test_write_well_metadata(self, temp_dir):
        from squid.backend.io.writers.zarr_writer import write_well_metadata
...
class TestSaveZarrJobWithSimulation:
    ...
    def test_save_zarr_job_simulated(self):
        ...
        original_compression = _def.SIMULATED_DISK_IO_COMPRESSION

        try:
            _def.SIMULATED_DISK_IO_ENABLED = True
            _def.SIMULATED_DISK_IO_COMPRESSION = True
            ...
            _def.SIMULATED_DISK_IO_COMPRESSION = original_compression
...
class TestSimulatedZarrWrite:
    ...
    def test_simulated_write_basic(self):
        ...
        import _def
        from squid.backend.io.io_simulation import simulated_zarr_write
        ...
        try:
            _def.SIMULATED_DISK_IO_ENABLED = True
            _def.SIMULATED_DISK_IO_SPEED_MB_S = 10000.0  # Very fast for tests
            _def.SIMULATED_DISK_IO_COMPRESSION = True
...
class TestHCSWorkflowIntegration:
    ...
    def test_hcs_plate_metadata_write_failure_propagates(self, temp_dir):
        ...
        with patch(
            "squid.backend.io.writers.zarr_writer.write_plate_metadata",
    ...
    def test_hcs_well_metadata_write_failure_propagates(self, temp_dir):
        ...
        with patch(
            "squid.backend.io.writers.zarr_writer.write_well_metadata",
...
class TestZarrPathConsistency:
    ...
    Both the UI layer and ZarrWriterInfo now use shared utility functions from
    squid.backend.io.writers.zarr_writer, ensuring path consistency. These tests
    verify the utility functions produce correct paths.
    """
    ...
    def test_hcs_paths_use_shared_utility(self):
        ...
        """
        from squid.backend.io.writers.zarr_writer import build_hcs_zarr_fov_path
    ...
    def test_non_hcs_per_fov_paths_use_shared_utility(self):
        ...
        ZarrWriterInfo.get_output_path returns the ARRAY path (appends /0).
        """
        from squid.backend.io.writers.zarr_writer import build_per_fov_zarr_path
    ...
    def test_6d_paths_use_shared_utility(self):
        ...
        from squid.backend.io.writers.zarr_writer import build_6d_zarr_path
...
class TestZarrWriterFovPositions:
    """Tests for per-FOV stage position metadata in zarr files."""
    ...
    def test_fov_position_recorded_in_metadata(self, temp_dir):
        """5D writer with a recorded position stores it in _squid.fov_positions."""
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter
        ...
        writer = ZarrWriter(config)
        writer.initialize()
        writer.record_fov_position(x_mm=1.5, y_mm=2.5, z_mm=0.1)
        ...
        attrs = zarr_json["attributes"]
        assert "fov_positions" in attrs["_squid"]
        positions = attrs["_squid"]["fov_positions"]
        assert len(positions) == 1
        assert positions[0]["fov"] == 0
        assert positions[0]["x_mm"] == 1.5
        assert positions[0]["y_mm"] == 2.5
        assert positions[0]["z_mm"] == 0.1

    def test_fov_position_translation_transform_5d(self, temp_dir):
        """5D writer adds OME-NGFF translation transform with position in micrometers."""
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter
        ...
        writer = ZarrWriter(config)
        writer.initialize()
        writer.record_fov_position(x_mm=10.0, y_mm=20.0, z_mm=0.5)
        writer.write_frame(np.zeros((32, 32), dtype=np.uint16), t=0, c=0, z=0)
        writer.finalize()
        ...
        assert len(transforms) == 2  # scale + translation
        assert transforms[0]["type"] == "scale"
        assert transforms[1]["type"] == "translation"
        # Values in micrometers, axis order: t, c, z, y, x
        assert transforms[1]["translation"] == [0.0, 0.0, 500.0, 20000.0, 10000.0]

    def test_fov_position_multiple_fovs_6d(self, temp_dir):
        """6D writer records multiple FOV positions in _squid.fov_positions."""
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter

        output_path = os.path.join(temp_dir, "test.zarr")
        config = ZarrAcquisitionConfig(
            output_path=output_path,
            shape=(3, 1, 1, 1, 32, 32),  # 6D: 3 FOVs
            dtype=np.uint16,
            pixel_size_um=1.0,
            is_hcs=False,
        )
        ...
        writer = ZarrWriter(config)
        writer.initialize()
        writer.record_fov_position(x_mm=0.0, y_mm=0.0, z_mm=0.0, fov=0)
        writer.record_fov_position(x_mm=1.0, y_mm=2.0, z_mm=0.1, fov=1)
        writer.record_fov_position(x_mm=3.0, y_mm=4.0, z_mm=0.2, fov=2)
        for fov in range(3):
            writer.write_frame(np.zeros((32, 32), dtype=np.uint16), t=0, c=0, z=0, fov=fov)
        writer.finalize()
        ...
        assert positions[0] == {"fov": 0, "x_mm": 0.0, "y_mm": 0.0, "z_mm": 0.0}
        assert positions[1] == {"fov": 1, "x_mm": 1.0, "y_mm": 2.0, "z_mm": 0.1}
        assert positions[2] == {"fov": 2, "x_mm": 3.0, "y_mm": 4.0, "z_mm": 0.2}

        # 6D should NOT have translation transform (multiple FOVs, no single origin)
        transforms = attrs["ome"]["multiscales"][0]["datasets"][0]["coordinateTransformations"]
        assert len(transforms) == 1  # scale only
        assert transforms[0]["type"] == "scale"

    def test_fov_position_idempotent(self, temp_dir):
        """Recording the same FOV twice keeps only the first value."""
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter
        ...
        writer = ZarrWriter(config)
        writer.initialize()
        writer.record_fov_position(x_mm=1.0, y_mm=2.0, z_mm=3.0)
        writer.record_fov_position(x_mm=99.0, y_mm=99.0, z_mm=99.0)  # Should be ignored
        writer.write_frame(np.zeros((32, 32), dtype=np.uint16), t=0, c=0, z=0)
        writer.finalize()
        ...
        positions = zarr_json["attributes"]["_squid"]["fov_positions"]
        assert len(positions) == 1
        assert positions[0]["x_mm"] == 1.0
        assert positions[0]["y_mm"] == 2.0
        assert positions[0]["z_mm"] == 3.0

    def test_no_position_no_metadata(self, temp_dir):
        """If no position is recorded, fov_positions key is absent from metadata."""
        from squid.backend.io.writers.zarr_writer import ZarrAcquisitionConfig, ZarrWriter
        ...
        assert "fov_positions" not in zarr_json["attributes"]["_squid"]
