...
class HamamatsuCamera(AbstractCamera):
    ...
    def _open(index=None, sn=None) -> Tuple[Dcam, HamamatsuCapabilities]:
        ...
        dcam_init, cam_count = Dcamapi.init()

        if not dcam_init:
            raise CameraError("Dcam api initialization failed.")
        ...
        default_width = camera.prop_getattr(DCAM_IDPROP.IMAGE_WIDTH)
        default_height = camera.prop_getattr(DCAM_IDPROP.IMAGE_HEIGHT)
    ...
    def __init__(
        ...
        # start_streaming and stop_streaming manage this thread.
        ...
        self._read_thread_wait_period_s = 0.1
    ...
    def _read_frames_when_available(self):
        ...
        while self._read_thread_keep_running.is_set():
            ...
            # to continue.
            try:
                if self._camera.wait_event(DCAMWAIT_CAPEVENT.FRAMEREADY, self._read_thread_wait_period_s / 1000.0):
    ...
    def start_streaming(self):
        with self._read_thread_lock:
            if self._read_thread is not None and self._read_thread_running.is_set():
                self._log.error("Read thread exists and thread is marked as running!")
                return False
            ...
            if not self._allocate_read_buffers():
                self._log.error("Couldn't allocate read buffers for streaming.")
                return False
    ...
    def stop_streaming(self):
        with self._read_thread_lock:
            if self._read_thread is None:
                self._log.warning("No read thread, already not streaming?")
                return True
    ...
    def set_region_of_interest(self, offset_x: int, offset_y: int, width: int, height: int):
        with self._pause_streaming():
            ...
            if not self._camera.prop_setvalue(DCAM_IDPROP.SUBARRAYHPOS, offset_x):
                fail("Could not set roi x offset.")

            if not self._camera.prop_setvalue(DCAM_IDPROP.SUBARRAYHSIZE, width):
                fail("Could not set roi width.")

            if not self._camera.prop_setvalue(DCAM_IDPROP.SUBARRAYVPOS, offset_y):
                fail("Could not set roi y offset.")

            if not self._camera.prop_setvalue(DCAM_IDPROP.SUBARRAYVSIZE, height):
