...
class HamamatsuCamera(AbstractCamera):
    ...
    def __init__(
        ...
        # buffers.
        self._camera_buffer_lock = threading.Lock()
    ...
    def _allocate_read_buffers(self, count=5):
        # Make sure any existing buffers are freed.
    ...
    def _read_frames_when_available(self):
        ...
        while self._read_thread_keep_running.is_set():
            ...
            # to continue.
            try:
                if self._camera.wait_event(DCAMWAIT_CAPEVENT.FRAMEREADY, int(round(self._read_thread_wait_period_s / 1000.0))):
                    # The dcam driver handles setting the correct width and height, so we can use the
                    # np frame directly.
                    with self._camera_buffer_lock:
                        ...
                        # It is unclear if the Hamamatsu gives us an np array that points
                        # to some of the buffers we allocated with it.  Just in case, make
                        # our own copy.
                        our_frame = np.copy(raw_frame)
    ...
    def read_camera_frame(self) -> CameraFrame:
        ...
        while self.get_frame_id() == starting_id:
            if time.time() > timeout_time_s:
                raise CameraError(f"Timed out after waiting {timeout_s=}[s] for frame ({starting_id=}), total_frame_time={self.get_total_frame_time()}.")
            time.sleep(0)
