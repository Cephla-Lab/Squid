...
class Stitcher(QThread, QObject):
    ...
    def __init__(
        ...
        self.is_reversed = self.determine_directions(
            self.input_folder
        )  # init: top to bottom, left to right
    ...
    def get_time_points(self, input_folder):
        try:  # detects directories named as integers, representing time points.
            time_points = [
                ...
                if os.path.isdir(os.path.join(input_folder, d)) and d.isdigit()
            ]
    ...
    def extract_acquisition_parameters(self, input_folder):
        acquistion_params_path = os.path.join(
            input_folder, "acquisition parameters.json"
        )
    ...
    def determine_directions(self, input_folder):
        ...
        coordinates = pd.read_csv(
            os.path.join(input_folder, self.time_points[0], "coordinates.csv")
        )
        try:
        ...
        k_rev = not coordinates.sort_values(by="z_level")[
            "z (um)"
    ...
    def parse_filenames(self, time_point):
        ...
        try:
            first_region, first_i, first_j, first_k, channel_name = os.path.splitext(
                first_filename
            )[0].split("_", 4)
            first_k = int(first_k)
            ...
            first_i, first_j, first_k, channel_name = os.path.splitext(first_filename)[
                0
            ].split("_", 3)
        ...
        for filename in sorted_input_files:
            if self.is_wellplate:
                region, i, j, k, channel_name = os.path.splitext(filename)[0].split(
                    "_", 4
                )
            else:
                region = "0"
            ...
            self.stitching_data.setdefault(region, {}).setdefault(
                channel_name, {}
            ).setdefault(k, {}).setdefault((i, j), tile_info)
        ...
        self.channel_colors = [
            CHANNEL_COLORS_MAP.get(self.extract_wavelength(name), {"hex": 0xFFFFFF})[
                "hex"
            ]
        ...
        for channel in self.channel_names:
            ...
            for roi in self.regions:
                for z_level in self.stitching_data[roi][channel]:
                    for row_col, tile_info in self.stitching_data[roi][channel][
                        z_level
                    ].items():
            ...
            if self.is_rgb[channel]:
                ...
                images_r = [
                    dask_imread(tile["filepath"])[0][:, :, 0] for tile in selected_tiles
                ]
                images_g = [
                    dask_imread(tile["filepath"])[0][:, :, 1] for tile in selected_tiles
                ]
                images_b = [
                    dask_imread(tile["filepath"])[0][:, :, 2] for tile in selected_tiles
                ]
                channel = channel.split("_")[0]
    ...
    def calculate_horizontal_shift(
        self, img1_path, img2_path, max_overlap, margin_ratio=0.2
    ):
        try:
            ...
            shift, error, diffphase = phase_cross_correlation(
                img1_overlap, img2_overlap, upsample_factor=10
            )
    ...
    def calculate_shifts(self, roi=""):
        ...
        self.registration_channel = (
            self.registration_channel
            if self.registration_channel in self.channel_names
        ...
        print(
            "objective calculated - vertical overlap:",
            self.max_y_overlap,
            ", horizontal overlap:",
            self.max_x_overlap,
        )
        ...
        for (row, col), tile_info in self.stitching_data[roi][
            self.registration_channel
        ][self.registration_z_level].items():
        ...
        self.v_shift = (
            self.calculate_vertical_shift(
                img1_path, img2_path_vertical, self.max_y_overlap
            )
            if self.max_y_overlap > 0
            and img2_path_vertical
            and img1_path != img2_path_vertical
            else (0, 0)
        )
        self.h_shift = (
            self.calculate_horizontal_shift(
                img1_path, img2_path_horizontal, self.max_x_overlap
            )
            if self.max_x_overlap > 0
            and img2_path_horizontal
            and img1_path != img2_path_horizontal
            else (0, 0)
        )
    ...
    def calculate_dynamic_shifts(self, roi, channel, z_level, row, col):
        ...
        if (row, col - 1) in self.stitching_data[roi][channel][z_level]:
            left_tile_path = self.stitching_data[roi][channel][z_level][row, col - 1][
                "filepath"
            ]
            current_tile_path = self.stitching_data[roi][channel][z_level][row, col][
                "filepath"
            ]
            # Calculate horizontal shift
            new_h_shift = self.calculate_horizontal_shift(
                left_tile_path, current_tile_path, abs(self.h_shift[1])
            )

            # Check if the new horizontal shift is within 10% of the precomputed shift
            if self.h_shift == (0, 0) or (
                0.95 * abs(self.h_shift[1])
                <= abs(new_h_shift[1])
                <= 1.05 * abs(self.h_shift[1])
                and 0.95 * abs(self.h_shift[0])
                <= abs(new_h_shift[0])
                <= 1.05 * abs(self.h_shift[0])
            ):
        ...
        if (row - 1, col) in self.stitching_data[roi][channel][z_level]:
            top_tile_path = self.stitching_data[roi][channel][z_level][row - 1, col][
                "filepath"
            ]
            current_tile_path = self.stitching_data[roi][channel][z_level][row, col][
                "filepath"
            ...
            new_v_shift = self.calculate_vertical_shift(
                top_tile_path, current_tile_path, abs(self.v_shift[0])
            )

            # Check if the new vertical shift is within 10% of the precomputed shift
            if self.v_shift == (0, 0) or (
                0.95 * abs(self.v_shift[0])
                <= abs(new_v_shift[0])
                <= 1.05 * abs(self.v_shift[0])
                and 0.95 * abs(self.v_shift[1])
                <= abs(new_v_shift[1])
                <= 1.05 * abs(self.v_shift[1])
            ):
    ...
    def init_output(self, time_point, region_id):
        ...
        self.output_path = os.path.join(
            output_folder,
            f"{region_id}_{self.output_name}"
            if self.is_wellplate
        ...
        if self.is_wellplate and STITCH_COMPLETE_ACQUISITION:
            ...
            self.num_pyramid_levels = math.ceil(
                np.log2(max(x_max, y_max) / 1024 * max(len(rows), len(columns)))
            )
        else:
    ...
    def stitch_images(self, time_point, roi, progress_callback=None):
        ...
        total_tiles = sum(
            len(z_data)
            for channel_data in self.stitching_data[roi].values()
        ...
        for z_level in range(self.num_z):
            for row in range(self.num_rows):
                ...
                for col in range(self.num_cols):
                    ...
                    if (
                        self.use_registration
                        and DYNAMIC_REGISTRATION
                        and z_level == self.registration_z_level
                    ):
                        if (row, col) in self.stitching_data[roi][
                            self.registration_channel
                        ][z_level]:
                            tile_info = self.stitching_data[roi][
                                self.registration_channel
                            ][z_level][(row, col)]
                    ...
                    for channel in self.channel_names:
                        if (row, col) in self.stitching_data[roi][channel][z_level]:
                            tile_info = self.stitching_data[roi][channel][z_level][
                                (row, col)
                            ]
                            ...
                            if self.is_rgb[channel]:
                                for color_idx, color in enumerate(["R", "G", "B"]):
                                    ...
                                    self.stitch_single_image(
                                        tile_color,
                                        z_level,
                                        self.mono_channel_names.index(color_channel),
                                        row,
                                        col,
                                    )
                                    processed_tiles += 1
                                ...
                                self.stitch_single_image(
                                    tile,
                                    z_level,
                                    self.mono_channel_names.index(channel),
                                    row,
                                    col,
                                )
                                processed_tiles += 1
    ...
    def stitch_single_image(self, tile, z_level, channel_idx, row, col):
        ...
        top_crop = (
            max(0, (-self.v_shift[0] // 2) - abs(self.h_shift[0]) // 2)
            if row > 0
            else 0
        )
        bottom_crop = (
        ...
        left_crop = (
            max(0, (-self.h_shift[1] // 2) - abs(self.v_shift[1]) // 2)
            if col > 0
            else 0
        )
        right_crop = (
        ...
        tile = tile[
            top_crop : tile.shape[0] - bottom_crop,
            left_crop : tile.shape[1] - right_crop,
        ]
        ...
        self.stitched_images[
            0, channel_idx, z_level, y : y + tile.shape[0], x : x + tile.shape[1]
        ] = tile
    ...
    def save_as_ome_tiff(self):
        ...
        self.acquisition_params.get("sensor_pixel_size_um", None)
        dims = "TCZYX"
        # if self.is_rgb:
        #     dims += "S"
        ...
        ome_metadata = OmeTiffWriter.build_ome(
            ...
            physical_pixel_sizes=[
                types.PhysicalPixelSizes(dz_um, self.pixel_size_um, self.pixel_size_um)
            ],
            # is_rgb=self.is_rgb
            # channel colors
        )
    ...
    def save_as_ome_zarr(self):
        ...
        zarr_writer.write_image(
            ...
            physical_pixel_sizes=types.PhysicalPixelSizes(
                dz_um, self.pixel_size_um, self.pixel_size_um
            ),
            channel_names=self.mono_channel_names,
            channel_colors=self.channel_colors,
            dimension_order=dims,
    ...
    def create_complete_ome_zarr(self):
        ...
        final_path = os.path.join(
            self.input_folder,
            self.output_name.replace(".ome.zarr", "")
            + "_complete_acquisition.ome.zarr",
        )
        if len(self.time_points) == 1:
            zarr_path = os.path.join(self.input_folder, "0_stitched", self.output_name)
    ...
    def create_hcs_ome_zarr(self):
        ...
        if len(self.time_points) == 1 and len(self.regions) == 1:
            stitched_zarr_path = os.path.join(
                self.input_folder, "0_stitched", f"{self.regions[0]}_{self.output_name}"
            )
            ...
            rows, columns = self.get_rows_and_columns()
            well_paths = [
                f"{well_id[0]}/{well_id[1:]}" for well_id in sorted(self.regions)
            ]
            print(well_paths)
            ome_zarr.writer.write_plate_metadata(
                root_group, rows, [str(col) for col in columns], well_paths
            )
        ...
        try:
            for time_point in self.time_points:
                ...
                for well in self.regions:
                    ...
                    print("\nstarting stitching...")
                    self.stitch_images(
                        time_point, well, progress_callback=self.update_progress.emit
                    )
            ...
            if (
                ...
                and not self.flexible
                and ".ome.zarr" in self.output_name
            ):
