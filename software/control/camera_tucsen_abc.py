...
from squid.abc import CameraFrame, CameraFrameFormat, CameraAcquisitionMode
...
class TucsenCamera(AbstractCamera):
    ...
    _GAIN_MODE_TO_LINE_RATE_US = {
        GainMode400BSIV3.HDR: 11.2,
        GainMode400BSIV3.CMS: 11.2,
        GainMode400BSIV3.HIGH_SPEED: 7.2,
        GainModeFL26BW.STANDARD: 34.67,
        GainModeFL26BW.LOW_NOISE: 69.3,
        GainModeFL26BW.SENBIN: 12.58,
    }
    ...
    def __init__(
        ...
        self.trigger_attr = TUCAM_TRIGGER_ATTR()  # We need to keep trigger attribute for starting and stopping streaming
        ...
        """
        # These are for FL26BW model. We'll work on it later
    ...
    def _wait_for_frame(self):
        ...
        while self._read_thread_keep_running.is_set():
            try:
                wait_time = self._read_thread_wait_period_s * 1000.0
    ...
    def set_exposure_time(self, exposure_time_ms: float):
        ...
        if TUCAM_Prop_SetValue(self._camera, TUCAM_IDPROP.TUIDP_EXPOSURETM.value, c_double(adjusted_exposure_time), 0) != TUCAMRET.TUCAMRET_SUCCESS:
    ...
    def _raw_set_resolution(self, bin_value: c_int):
        with self._pause_streaming():
            if TUCAM_Capa_SetValue(self._camera, TUCAM_IDCAPA.TUIDC_RESOLUTION.value, bin_value) != TUCAMRET.TUCAMRET_SUCCESS:
    ...
    def set_binning(self, binning_factor_x: int, binning_factor_y: int):
        if not (binning_factor_x, binning_factor_y) in self.binning_options:
            raise CameraError(f"No binning option exists for {binning_factor_x}x{binning_factor_y}")
        self._raw_set_resolution(self.binning_options[(binning_factor_x, binning_factor_y)])
    ...
    def get_resolution(self):
        ...
        return self.binning_options[idx.value]
    ...
    def set_region_of_interest(self, offset_x: int, offset_y: int, width: int, height: int):
        ...
        with self._pause_streaming():
            if TUCAM_Cap_SetROI(self._camera, roi_attr) != TUCAMRET.TUCAMRET_SUCCESS:
                raise CameraError(f"Failed to set ROI: {roi_attr.nHOffset}, {roi_attr.nVOffset}, {roi_attr.nWidth}, {roi_attr.nHeight}")
            self._update_internal_settings()
