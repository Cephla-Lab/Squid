...
class MultiPointWorker(QObject):
    ...
    def run_grid_acquisition(self, current_path):
        ...
        for region_id in range(n_regions):
            ...
            output_queue = deque()
            streaming_callback = self.get_image_processor_fn(streaming_camera_queue, output_queue)
            ...
            for i in range(self.NY):
                ...
                for j in range(self.NX):
                    ...
                    if not self.multiPointController.scanCoordinates or (real_i, real_j) not in self.multiPointController.scanCoordinates.grid_skip_positions:
                        # IMO: Move to position
                        # IMO: Trigger and tell camera processor to expect an image.
                        self.acquire_at_position(region_id, current_path, fov_count, streaming_camera_queue, i=real_i, j=real_j)
                        fov_count += 1
