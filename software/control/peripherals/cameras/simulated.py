...
from control.peripherals.cameras.camera_utils import camera_registry
...
class SimulatedFocusCamera(SimulatedCameraBase):
    ...
    def set_event_bus(self, event_bus) -> None:
        ...
        if event_bus is not None:
            from squid.events import StagePositionChanged, PiezoPositionChanged
...
class SimulatedMainCamera(SimulatedCameraBase):
    ...
    def _get_cell_renderer(self):
        ...
        if self._cell_renderer is None:
            from control.peripherals.cameras.cell_renderer import SimpleCellFieldRenderer
    ...
    def set_event_bus(self, event_bus) -> None:
        ...
        self._event_bus = event_bus
        if event_bus is not None:
            from squid.events import StagePositionChanged, ObjectiveChanged
            event_bus.subscribe(StagePositionChanged, self._on_stage_position_changed)
