...
class SquidFilterWheel(AbstractFilterWheelController):
    def __init__(
        ...
        if microcontroller is None:
            raise Exception(
                "Error, microcontroller is need by the SquidFilterWheelWrapper"
            )
        ...
        if HAS_ENCODER_W:
            self.microcontroller.set_pid_arguments(
                self._config.motor_slot_index, PID_P_W, PID_I_W, PID_D_W
            )
            self.microcontroller.configure_stage_pid(
                self._config.motor_slot_index,
                self._config.transitions_per_revolution,
                ENCODER_FLIP_DIR_W,
            )
            self.microcontroller.turn_on_stage_pid(
                self._config.motor_slot_index, ENABLE_PID_W
            )
    ...
    def previous_position(self):
        if self.w_pos_index > self._config.min_index:
            self.move_w(
                -(
                    SCREW_PITCH_W_MM
                    / (self._config.max_index - self._config.min_index + 1)
                )
            )
            self.microcontroller.wait_till_operation_is_completed()
            self.w_pos_index -= 1
