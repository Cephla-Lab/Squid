...
class MicroscopeAddons:
    ...
    def build_from_global_config(
        ...
        xlight = None
        if (
            control._def.ENABLE_SPINNING_DISK_CONFOCAL
            and not control._def.USE_DRAGONFLY
        ):
        ...
        if fw_config:
            emission_filter_wheel = (
                control.peripherals.filter_wheel.utils.get_filter_wheel_controller(
        ...
        if control._def.USE_XERYON:
            objective_changer = (
                ObjectiveChanger2PosController(
                    sn=control._def.XERYON_SERIAL_NUMBER, stage=stage
                )
                if not simulated
        ...
        if control._def.RUN_FLUIDICS:
            fluidics = Fluidics(
                config_path=control._def.FLUIDICS_CONFIG_PATH, simulation=simulated
            )
...
class LowLevelDrivers:
    ...
    def build_from_global_config(simulated: bool = False) -> "LowLevelDrivers":
        micro_serial_device = (
            ...
            if not simulated
            else control.microcontroller.get_microcontroller_serial_device(
                simulated=True
            )
        )
    ...
    def prepare_for_use(self) -> None:
        if self.microcontroller and control._def.HAS_OBJECTIVE_PIEZO:
            ...
            control._def.OUTPUT_GAINS.CHANNEL7_GAIN = (
                control._def.OBJECTIVE_PIEZO_CONTROL_VOLTAGE_RANGE == 5
            )
            div = 1 if control._def.OUTPUT_GAINS.REFDIV else 0
            gains = sum(
                getattr(control._def.OUTPUT_GAINS, f"CHANNEL{i}_GAIN") << i
                for i in range(8)
            )
...
class Microscope:
    ...
    def _prepare_for_use(self) -> None:
        ...
        self.camera.set_pixel_format(
            squid.config.CameraPixelFormat.from_string(
        ...
        if self.addons.camera_focus:
            self.addons.camera_focus.set_pixel_format(
                squid.config.CameraPixelFormat.from_string("MONO8")
            )
