            ...
            try:
                ...
                if job is not None:
                    ...
                    if self._bp_pending_jobs is not None:
                        ...
                        # Calculate and decrement image bytes immediately when job completes.
                        # Note: For DownsampledViewJob, the image data moves to subprocess memory
                        # when the job is processed, so it's correct to release the bytes from
                        # backpressure tracking at this point (not when the well completes).
                        image_bytes = 0
                        if job.capture_image and job.capture_image.image_array is not None:
                            image_bytes = job.capture_image.image_array.nbytes
