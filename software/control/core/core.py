    ...
    def add_manual_region(self, shape_coords, overlap_percent):
        ...
        for x_center, y_center in grid_points:
            if not self.validate_coordinates(x_center, y_center):
                ...
                continue
            if not self._is_in_polygon(x_center, y_center) and not any([self._is_in_polygon(x_corner, y_corner, shape_coords) for (x_corner, y_corner) in corners(x_center, y_center, fov_size_mm)]):
