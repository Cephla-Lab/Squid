...
def _find_config_files(pattern: str) -> list[str]:
    """Search for config files in the current working directory, then project root."""
...
FOCUS_LOCK_PIEZO_CRITICAL_MARGIN_UM = 10.0
FOCUS_LOCK_DEFAULT_MODE = "off"
FOCUS_LOCK_GAIN_SIGMA = 0.5
FOCUS_LOCK_KI = 0.05
FOCUS_LOCK_INTEGRAL_LIMIT_UM = 2.0
FOCUS_LOCK_MAX_NAN_HOLDOVER_CYCLES = 3
FOCUS_LOCK_NAN_HOLDOVER_DECAY = 0.5
...
def load_formats():
    ...
    default_path = FORMATS_DIR

    # Load objectives (from default location)
    objectives = read_objectives_csv(default_path / "objectives.csv")

    # Try cache first for sample formats, fall back to default if not found
    cached_formats_path = cache_path / "sample_formats.csv"
    default_formats_path = default_path / "sample_formats.csv"
...
try:
    with open(CACHE_DIR / "config_file_path.txt", "r") as file:
        ...
        if cached_path:
            ...
            CACHED_CONFIG_FILE_PATH = str(cached_path)
except FileNotFoundError:
    CACHED_CONFIG_FILE_PATH = None

config_files = _find_config_files("configuration*.ini")
if config_files:
    ...
    cached_config_path = Path(config_files[0]).resolve()
    with open(CACHE_DIR / "config_file_path.txt", "w") as file:
        file.write(str(cached_config_path))
    CACHED_CONFIG_FILE_PATH = str(cached_config_path)
else:
    log.warning("configuration*.ini file not found, defaulting to legacy configuration")
    config_files = _find_config_files("configuration*.txt")
    if config_files:
        if len(config_files) > 1:
            log.error(
                "multiple machine configuration files found, the program will exit"
...
try:
    with open(CACHE_DIR / "objective_and_sample_format.txt", "r") as f:
