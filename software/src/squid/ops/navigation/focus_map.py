...
from squid.ops.navigation.scan_coordinates import FovCenter, ScanCoordinates
...
class NavigationViewer(QFrame):
    ...
    def load_background_image(self, image_path: str) -> None:
        ...
        self.background_image = cv2.imread(str(path))
        if self.background_image is None:
            fallback = Path(self.image_paths.get("glass slide", "assets/images/slide carrier_828x662.png"))
            if not fallback.is_absolute():
                fallback = PROJECT_ROOT / fallback
            self.background_image = cv2.imread(str(fallback))
        if self.background_image is None:
            raise ValueError(f"Failed to load navigation background image from {path}")
    ...
    def draw_fov_current_location(self, pos: Optional[squid.core.abc.Pos]) -> None:
        if not pos:
