...
from squid.core.config import CameraConfig, CameraPixelFormat, TucsenCameraModel
...
from squid.mcs.drivers.cameras.tucam_sdk import *
...
class TucsenCamera(AbstractCamera):
    ...
    def _open(index: Optional[int] = None, sn: Optional[str] = None) -> c_void_p:
        log = squid.core.logging.get_logger("TucsenCamera._open")
    ...
    def set_region_of_interest(
        ...
        if self._model_properties.is_genicam:
            nHOffset = squid.core.utils.hardware_utils.truncate_to_interval(offset_x, 8)
            nVOffset = squid.core.utils.hardware_utils.truncate_to_interval(offset_y, 2)
            nWidth = squid.core.utils.hardware_utils.truncate_to_interval(width, 8)
            nHeight = squid.core.utils.hardware_utils.truncate_to_interval(height, 2)
        else:
            ...
            roi_attr.nHOffset = squid.core.utils.hardware_utils.truncate_to_interval(offset_x, 4)
            roi_attr.nVOffset = squid.core.utils.hardware_utils.truncate_to_interval(offset_y, 4)
            roi_attr.nWidth = squid.core.utils.hardware_utils.truncate_to_interval(width, 4)
            roi_attr.nHeight = squid.core.utils.hardware_utils.truncate_to_interval(height, 4)
    ...
    def _set_genicam_parameter(
        ...
        Example:
            from squid.mcs.drivers.cameras.tucam_sdk import TUELEM_TYPE
