...
TOUPCAM_OPTION_FRAMERATE = (
    0x11  # limit the frame rate, the default value 0 means no limit
)
...
TOUPCAM_OPTION_CALLBACK_THREAD = (
    0x30  # dedicated thread for callback: 0 => disable, 1 => enable, default: 0
)
...
TOUPCAM_OPTION_ANTI_SHUTTER_EFFECT = (
    0x4B  # anti shutter effect: 1 => enable, 0 => disable; default: 0
)
...
TOUPCAM_OPTION_PSEUDO_COLOR_ENABLE = (
    0x65  # Pseudo: -1 => custom: use startcolor & endcolor to generate the colormap
)
#     0 => disable
#     1 => spot
#     2 => spring
...
#     26 => blue
#
TOUPCAM_OPTION_LOW_POWERCONSUMPTION = (
    0x66  # Low Power Consumption: 0 => disable, 1 => enable
)
...
#         ((val & 0xff0000) >> 16): average number
#
TOUPCAM_OPTION_OVEREXP_POLICY = (
    0x68  # Auto exposure over exposure policy: when overexposed,
)
...
# Default: 0
TOUPCAM_OPTION_READOUT_MODE = (
    0x69  # Readout mode: 0 = IWR (Integrate While Read), 1 = ITR (Integrate Then Read)
)
...
TOUPCAM_OPTION_TAILLIGHT = (
    0x6A  # Turn on/off tail Led light: 0 => off, 1 => on; default: on
)
...
TOUPCAM_OPTION_LOW_POWER_EXPOTIME = (
    0x6F  # Low Power Consumption: Enable if exposure time is greater than the set value
)
TOUPCAM_OPTION_ZERO_OFFSET = (
    0x70  # Sensor output offset to zero: 0 => disable, 1 => eanble; default: 0
)
TOUPCAM_OPTION_GVCP_TIMEOUT = (
    0x71  # GVCP Timeout: millisecond, range = [3, 75], default: 15
)
...
TOUPCAM_OPTION_RESET_SEQ_TIMESTAMP = (
    0x74  # Reset to 0: 1 => seq; 2 => timestamp; 3 => both
)
TOUPCAM_OPTION_TRIGGER_CANCEL_MODE = (
    0x75  # Trigger cancel mode: 0 => no frame, 1 => output frame; default: 0
)
TOUPCAM_OPTION_MECHANICALSHUTTER = (
    0x76  # Mechanical shutter: 0 => open, 1 => close; default: 0
)
...
TOUPCAM_IOCONTROLTYPE_GET_PWMSOURCE = (
    0x1D  # PWM Source: 0x00 => Opto-isolated input, 0x01 => GPIO0, 0x02 => GPIO1
)
...
TOUPCAM_IOCONTROLTYPE_GET_INPUT_STATE = (
    0x3B  # Input state: 0 (low level) or 1 (high level)
)
...
TOUPCAM_IOCONTROLTYPE_GET_DEBOUNCER_TRIGGER_NUMBER = (
    0x45  # Trigger signal number after debounce
)
TOUPCAM_IOCONTROLTYPE_GET_EFFECTIVE_TRIGGER_NUMBER = (
    0x47  # Effective trigger signal number
)
...
TOUPCAM_AAF_GETTEMP = (
    0x14  # in 0.1 degrees Celsius, such as: 32 means 3.2 degrees Celsius
)
...
class ToupcamAFState:
    def __init__(
        ...
        self.AF_LensAP_Update_Flag = (
            AF_LensAP_Update_Flag  # mark for whether the lens aperture is calibrated
        )
...
class Toupcam:
    ...
    class __AFState(ctypes.Structure):
        _fields_ = [
            ...
            (
                "AF_LensAP_Update_Flag",
                ctypes.c_byte,
            ),  # mark for whether the lens aperture is calibrated
    ...
    def PullImageV4(self, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Toupcam_PullImageV4(
    ...
    def WaitImageV4(self, nWaitMS, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Toupcam_WaitImageV4(
    ...
    def TriggerSyncV4(self, nWaitMS, pImageData, bits, rowPitch, pInfo):
        ...
        """
        if pInfo is None:
            self.__lib.Toupcam_TriggerSyncV4(
                self.__h, nWaitMS, pImageData, bits, rowPitch, None
            )
        else:
            x = self.__FrameInfoV4()
            self.__lib.Toupcam_TriggerSyncV4(
                self.__h, nWaitMS, pImageData, bits, rowPitch, ctypes.byref(x)
            )
            self.__convertFrameInfoV4(pInfo, x)
    ...
    def TriggerSync(self, nWaitMS, pImageData, bits, rowPitch, pInfo):
        ...
        """
        if pInfo is None:
            self.__lib.Toupcam_TriggerSync(
                self.__h, nWaitMS, pImageData, bits, rowPitch, None
            )
        else:
            x = self.__FrameInfoV3()
    ...
    def put_ColorMatrix(self, v):
        if len(v) == 9:
            (ctypes.c_double * 9)(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8])
    ...
    def get_TecTargetRange(self):
        ...
        x = ctypes.c_int(0)
        self.__lib.Toupcam_get_Option(
            self.__h, TOUPCAM_OPTION_TECTARGET_RANGE, ctypes.byref(x)
        )
    ...
    def put_RoiN(self, xOffset, yOffset, xWidth, yHeight):
        """multiple Roi"""
        if (
            len(xOffset) != len(yOffset)
            or len(xOffset) != len(xWidth)
            or len(xOffset) != len(yHeight)
        ):
            raise HRESULTException(0x80070057)
        Num = len(xOffset)
        ...
        self.__lib.Toupcam_put_RoiN(
            self.__h, pxOffset, pyOffset, pxWidth, pyHeight, Num
        )
    ...
    def __initlib(cls):
        if cls.__lib is None:
            ...
            if sys.platform == "win32":
                cls.__ModelV2._fields_ = [  # camera model v2 win32
                    (
                        "name",
                        ctypes.c_wchar_p,
                    ),  # model name, in Windows, we use unicode
            ...
            if sys.platform == "win32":
                ...
                cls.__lib.Toupcam_put_Name.argtypes = [
                    ctypes.c_wchar_p,
                    ctypes.c_char_p,
                ]
                cls.__lib.Toupcam_get_Name.argtypes = [
                    ctypes.c_wchar_p,
                    ctypes.c_char * 64,
                ]
            ...
            cls.__lib.Toupcam_query_Name.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char * 64,
            ]
            ...
            if sys.platform == "win32":
                ...
                cls.__lib.Toupcam_FpncExport.argtypes = [
                    ctypes.c_void_p,
                    ctypes.c_wchar_p,
                ]
                cls.__lib.Toupcam_FpncImport.argtypes = [
                    ctypes.c_void_p,
                    ctypes.c_wchar_p,
                ]
            else:
            ...
            cls.__lib.Toupcam_put_XY.argtypes = [
                ctypes.c_void_p,
                ctypes.c_int,
                ctypes.c_int,
            ]
            ...
            cls.__lib.Toupcam_get_FocusMotor.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(cls.__FocusMotor),
            ]
