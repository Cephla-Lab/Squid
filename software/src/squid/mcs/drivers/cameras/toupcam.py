...
import threading
import squid.mcs.drivers.cameras.toupcam_sdk as toupcam
from squid.mcs.drivers.cameras.toupcam_exceptions import hresult_checker
...
class ToupcamCamera(AbstractCamera):
    ...
    def _calculate_strobe_info(
        ...
        try:
            ...
            log.error(
                f"get max_framerate fail --> {squid.mcs.drivers.cameras.toupcam_exceptions.explain(ex)}"
            )
            raise
        ...
        try:
            ...
            log.exception(
                f"put max_framerate fail --> {squid.mcs.drivers.cameras.toupcam_exceptions.explain(ex)}"
            )
            raise
