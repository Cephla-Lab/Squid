...
from squid.mcs.drivers.lighting.led import (
    LightSourceType,
...
from squid.mcs.drivers.lighting import SciMicroscopyLEDArray
...
import squid.mcs.drivers.lighting.celesta
import squid.mcs.drivers.lighting.illumination_andor
...
import squid.mcs.drivers.filter_wheels.utils
import squid.core.logging
import squid.mcs.drivers.stages.cephla
import squid.mcs.drivers.stages.stage_utils

if _def.USE_XERYON:
    from squid.mcs.drivers.peripherals.objective_changer import (
...
if _def.ENABLE_NL5:
    import squid.mcs.drivers.peripherals.nl5 as NL5
else:
    NL5 = TypeVar("NL5")
...
class MicroscopeAddons:
    ...
    def build_from_global_config(
        ...
        xlight = None
        if (
            _def.ENABLE_SPINNING_DISK_CONFOCAL
            and not _def.USE_DRAGONFLY
        ):
        ...
        if fw_config:
            emission_filter_wheel = (
                squid.mcs.drivers.filter_wheels.utils.get_filter_wheel_controller(
        ...
        if _def.SUPPORT_LASER_AUTOFOCUS:
            camera_focus = squid.mcs.drivers.cameras.camera_utils.get_camera(
...
class LowLevelDrivers:
    ...
    def build_from_global_config(simulated: bool = False) -> "LowLevelDrivers":
        micro_serial_device = (
            squid.mcs.microcontroller.get_microcontroller_serial_device(
                version=_def.CONTROLLER_VERSION, sn=_def.CONTROLLER_SN
            )
            if not simulated
            else squid.mcs.microcontroller.get_microcontroller_serial_device(
                simulated=True
            )
        )
        micro = squid.mcs.microcontroller.Microcontroller(
...
class Microscope:
    ...
    def build_from_global_config(
        ...
        camera = squid.mcs.drivers.cameras.camera_utils.get_camera(
        ...
        if _def.USE_LDI_SERIAL_CONTROL and not simulated:
            ...
            celesta = squid.mcs.drivers.lighting.celesta.CELESTA()
            illumination_controller = IlluminationController(
                low_level_devices.microcontroller,
            ...
            andor_laser = squid.mcs.drivers.lighting.illumination_andor.AndorLaser(
