...
class ConfigRepository:
    ...
    def __init__(self, base_path: Optional[Path] = None):
        if base_path is None:
            # Default: software/ directory (5 levels up from this file at
            # software/src/squid/core/config/repository.py)
    ...
    def load_profile(self, profile: str, objectives: Optional[List[str]] = None) -> None:
        ...
        try:
            from squid.core.config.default_config_generator import ensure_default_configs

            obj_list = objectives
            if obj_list is None:
                try:
                    import _def
                    obj_list = list(_def.OBJECTIVES) if hasattr(_def, "OBJECTIVES") else None
                except ImportError:
                    pass
    ...
    def profile_has_configs(self, profile: Optional[str] = None) -> bool:
        """Check if a profile has configuration files (general.yaml exists)."""
        profile_path = self._get_profile_path(profile)
    ...
    def get_confocal_config(self) -> Optional[ConfocalConfig]:
        """Load confocal configuration (cached). Returns None if no confocal."""
        cache_key = "confocal"
        if cache_key not in self._machine_cache:
    ...
    def resolve_wheel_reference(self, ref: FilterWheelReference) -> Optional[FilterWheelDefinition]:
        ...
        logger.warning(f"Filter wheel reference not found: {ref}. {available_info}.")
        return None
    ...
    def update_channel_setting(
        ...
        - "ZOffset" -> z_offset_um (top-level field on AcquisitionChannel)
        """
        profile = profile or self._current_profile
        if not profile:
