...
def validate_filter_wheel_list(wheels: List["FilterWheelDefinition"], context: str = "Filter wheel") -> None:
    """Validate a list of filter wheels for uniqueness and completeness."""
    if len(wheels) == 0:
        return
...
class FilterWheelDefinition(BaseModel):
    ...
    type: FilterWheelType = Field(..., description="Filter wheel type: excitation or emission")
