...
Ported from upstream commit ef375f28 with import path adjustments.
"""
...
def _default_controller_port_mapping() -> Dict[str, int]:
    """Build default controller port mapping from _def constants.

    Falls back to hardcoded values if _def is not available (e.g., in tests).
    """
    try:
        ...
        port_attrs = {
            "USB1": "ILLUMINATION_SOURCE_LED_ARRAY_FULL",
            "USB2": "ILLUMINATION_SOURCE_LED_ARRAY_LEFT_HALF",
            "USB3": "ILLUMINATION_SOURCE_LED_ARRAY_RIGHT_HALF",
            "USB4": "ILLUMINATION_SOURCE_LED_ARRAY_LEFTB_RIGHTR",
            "USB5": "ILLUMINATION_SOURCE_LED_ARRAY_LOW_NA",
            "USB7": "ILLUMINATION_SOURCE_LED_ARRAY_TOP_HALF",
            "USB8": "ILLUMINATION_SOURCE_LED_ARRAY_BOTTOM_HALF",
        }
        for port, attr in port_attrs.items():
            val = getattr(ILLUMINATION_CODE, attr, None)
            if val is not None:
                mapping[port] = val

        # Laser ports (D1-D5) - use D-port specific constants if available,
        # otherwise fall back to wavelength-based constants
        d_port_attrs = {
            "D1": "ILLUMINATION_D1",
            "D2": "ILLUMINATION_D2",
            "D3": "ILLUMINATION_D3",
        ...
        wavelength_fallbacks = {
            "D1": "ILLUMINATION_SOURCE_405NM",
            "D2": "ILLUMINATION_SOURCE_488NM",
            "D3": "ILLUMINATION_SOURCE_638NM",
            "D4": "ILLUMINATION_SOURCE_561NM",
            "D5": "ILLUMINATION_SOURCE_730NM",
        }
        for port, attr in d_port_attrs.items():
            val = getattr(ILLUMINATION_CODE, attr, None)
            if val is None:
                fallback = wavelength_fallbacks.get(port)
                if fallback:
                    val = getattr(ILLUMINATION_CODE, fallback, None)
            if val is not None:
                mapping[port] = val

        return mapping
    except ImportError:
        # Fallback for test environments without _def
        return {
            "USB1": 0,
            "USB2": 1,
            "USB3": 2,
...
class IlluminationChannelConfig(BaseModel):
    ...
    controller_port_mapping: Dict[str, int] = Field(
        default_factory=_default_controller_port_mapping,
    ...
    def get_available_ports(self) -> List[str]:
        """Get list of controller ports that have mappings.
