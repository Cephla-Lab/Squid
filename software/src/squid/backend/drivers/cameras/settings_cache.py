...
Ported from upstream commit 98c7fbd6.
...
def _get_cache_path() -> Path:
    """Get the cache file path, creating parent dir if necessary."""
    cache_dir = Path(__file__).parent.parent.parent.parent.parent.parent / "cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / "camera_settings.yaml"
...
def save_camera_settings(camera: AbstractCamera, cache_path: Optional[Path] = None) -> None:
...
def load_camera_settings(cache_path: Optional[Path] = None) -> Optional[CachedCameraSettings]:
    ...
    try:
        ...
        _log.error(
            f"Camera settings cache file is corrupted at {cache_path}: {e}. "
