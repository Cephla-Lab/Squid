...
class TUIMG_OPEN(Structure):
    _fields_ = [("pszfileName", c_void_p), ("hIdxTUImg", c_void_p)]
...
class TUCAM_REC_SAVE(Structure):
    _fields_ = [("nCodec", c_int32), ("pstrSavePath", c_char_p), ("fFps", c_float)]


# the register read/write struct
class TUCAM_REG_RW(Structure):
    _fields_ = [("nRegType", c_int32), ("pBuf", c_char_p), ("nBufSize", c_int32)]


# the subtract background struct
class TUCAM_IMG_BACKGROUND(Structure):
    _fields_ = [("bEnable", c_int32), ("ImgHeader", TUCAM_RAWIMG_HEADER)]


# the math struct
class TUCAM_IMG_MATH(Structure):
    _fields_ = [("bEnable", c_int32), ("nMode", c_int32), ("usGray", c_ushort)]
...
class TUCAM_UNION(Union):
    _fields_ = [("Int64", TUCAM_VALUEINT), ("Double", TUCAM_VALUEDOUBLE)]
...
TUCAM_Index_GetColorTemperature.argtypes = [
    c_void_p,
    c_int32,
    c_int32,
    c_int32,
...
TUCAM_GenICam_ElementAttr.argtypes = [
    c_void_p,
    POINTER(TUCAM_ELEMENT),
    c_void_p,
    c_int32,
]
...
TUCAM_GenICam_ElementAttrNext.argtypes = [
    c_void_p,
    POINTER(TUCAM_ELEMENT),
    c_void_p,
    c_int32,
