...
from squid.backend.drivers.cameras.toupcam_exceptions import hresult_checker
from squid.backend.drivers.cameras.camera_utils import camera_registry
...
class ToupcamCamera(AbstractCamera):
    ...
    def _calculate_strobe_info(
        ...
        log = squid.core.logging.get_logger("ToupcamCamera._calculate_strobe_delay")
        ...
        try:
            ...
            log.error(
                f"get max_framerate fail --> {squid.backend.drivers.cameras.toupcam_exceptions.explain(ex)}"
            )
            raise
        ...
        try:
            ...
            log.exception(
                f"put max_framerate fail --> {squid.backend.drivers.cameras.toupcam_exceptions.explain(ex)}"
            )
            raise
    ...
    def _open(index=None, sn=None) -> Tuple[toupcam.Toupcam, ToupCamCapabilities]:
        log = squid.core.logging.get_logger("ToupcamCamera._open")
    ...
    def set_region_of_interest(
        ...
        roi_offset_x = squid.core.utils.hardware_utils.truncate_to_interval(offset_x, 2)
        roi_offset_y = squid.core.utils.hardware_utils.truncate_to_interval(offset_y, 2)
        roi_width = squid.core.utils.hardware_utils.truncate_to_interval(width, 2)
        roi_height = squid.core.utils.hardware_utils.truncate_to_interval(height, 2)
        with self._pause_streaming():
            try:
