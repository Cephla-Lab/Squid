...
class DeviceManager(object):
    ...
    def open_device_by_index(self, index, access_mode=GxAccessMode.CONTROL):
        ...
        if device_class == GxDeviceClassList.U3V:
            ...
            raise NotFoundDevice(
                "DeviceManager.open_device_by_index: Does not support this device type."
            )
    ...
    def open_device_by_sn(self, sn, access_mode=GxAccessMode.CONTROL):
        ...
        if device_class == GxDeviceClassList.U3V:
            ...
            raise NotFoundDevice(
                "DeviceManager.open_device_by_sn: Does not support this device type."
            )
    ...
    def open_device_by_user_id(self, user_id, access_mode=GxAccessMode.CONTROL):
        ...
        if device_class == GxDeviceClassList.U3V:
            ...
            raise NotFoundDevice(
                "DeviceManager.open_device_by_user_id: Does not support this device type."
            )
...
class Device:
    ...
    def __init__(self, handle):
        ...
        self.FrameBufferOverwriteActive = BoolFeature(
            self.__dev_handle, GxFeatureID.BOOL_FRAME_STORE_COVER_ACTIVE
        )
...
class DataStream:
    def __init__(self, handle):
        self.__dev_handle = handle
        self.StreamAnnouncedBufferCount = IntFeature(
            self.__dev_handle, GxFeatureID.INT_ANNOUNCED_BUFFER_COUNT
        )
        self.StreamDeliveredFrameCount = IntFeature(
            self.__dev_handle, GxFeatureID.INT_DELIVERED_FRAME_COUNT
        )
        self.StreamLostFrameCount = IntFeature(
            self.__dev_handle, GxFeatureID.INT_LOST_FRAME_COUNT
        )
        self.StreamIncompleteFrameCount = IntFeature(
            self.__dev_handle, GxFeatureID.INT_INCOMPLETE_FRAME_COUNT
        )
        self.StreamDeliveredPacketCount = IntFeature(
            self.__dev_handle, GxFeatureID.INT_DELIVERED_PACKET_COUNT
        )
        self.payload_size = 0
...
class U3VDataStream(DataStream):
    def __init__(self, handle):
        ...
        self.StreamTransferSize = IntFeature(
            self.__handle, GxFeatureID.INT_STREAM_TRANSFER_SIZE
        )
        self.StreamTransferNumberUrb = IntFeature(
            self.__handle, GxFeatureID.INT_STREAM_TRANSFER_NUMBER_URB
        )
...
class GEVDataStream(DataStream):
    def __init__(self, handle):
        self.__handle = handle
        DataStream.__init__(self, self.__handle)
        self.StreamResendPacketCount = IntFeature(
            self.__handle, GxFeatureID.INT_RESEND_PACKET_COUNT
        )
        self.StreamRescuedPacketCount = IntFeature(
            self.__handle, GxFeatureID.INT_RESCUED_PACKED_COUNT
        )
        self.StreamResendCommandCount = IntFeature(
            self.__handle, GxFeatureID.INT_RESEND_COMMAND_COUNT
        )
        self.StreamUnexpectedPacketCount = IntFeature(
            self.__handle, GxFeatureID.INT_UNEXPECTED_PACKED_COUNT
        )
        self.MaxPacketCountInOneBlock = IntFeature(
            self.__handle, GxFeatureID.INT_MAX_PACKET_COUNT_IN_ONE_BLOCK
        )
        self.MaxPacketCountInOneCommand = IntFeature(
            self.__handle, GxFeatureID.INT_MAX_PACKET_COUNT_IN_ONE_COMMAND
        )
        self.ResendTimeout = IntFeature(self.__handle, GxFeatureID.INT_RESEND_TIMEOUT)
        self.MaxWaitPacketCount = IntFeature(
            self.__handle, GxFeatureID.INT_MAX_WAIT_PACKET_COUNT
        )
        self.ResendMode = EnumFeature(self.__handle, GxFeatureID.ENUM_RESEND_MODE)
        self.StreamMissingBlockIDCount = IntFeature(
            self.__handle, GxFeatureID.INT_MISSING_BLOCK_ID_COUNT
        )
        self.BlockTimeout = IntFeature(self.__handle, GxFeatureID.INT_BLOCK_TIMEOUT)
        self.MaxNumQueueBuffer = IntFeature(
            self.__handle, GxFeatureID.INT_MAX_NUM_QUEUE_BUFFER
        )
...
class RGBImage:
    ...
    def image_improvement(
        self, color_correction_param=0, contrast_lut=None, gamma_lut=None
    ):
        """
...
class RawImage:
    ...
    def convert(
        ...
        if (
            ...
            or pixel_bit_depth > GxPixelSizeEntry.BPP12
        ):
        ...
        if mode == "RAW8":
            ...
            return self.__raw8_to_rgb(
                image_raw8, convert_type, pixel_color_filter, flip
            )
        else:
    ...
    def get_numpy_array(self):
        ...
        if self.frame_data.pixel_format & PIXEL_BIT_MASK == GX_PIXEL_8BIT:
            image_np = numpy.frombuffer(
                self.__image_array, dtype=numpy.ubyte, count=image_size
            ).reshape(self.frame_data.height, self.frame_data.width)
        elif self.frame_data.pixel_format & PIXEL_BIT_MASK == GX_PIXEL_16BIT:
            image_np = numpy.frombuffer(
                self.__image_array, dtype=numpy.uint16, count=image_size
            ).reshape(self.frame_data.height, self.frame_data.width)
        else:
            image_np = None
