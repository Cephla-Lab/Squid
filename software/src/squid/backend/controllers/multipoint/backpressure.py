...
- be019383: Lifecycle enforcement, TOCTOU prevention, pre-warming
"""
...
class BackpressureController:
    ...
    Usage:
        ...
        runner = JobRunner(
            backpressure_jobs=bp_values[0],
            backpressure_bytes=bp_values[1],
            backpressure_event=bp_values[2],
        )
    ...
    def __init__(
        ...
        Args:
            ...
            bp_values: Pre-created backpressure values for sharing with pre-warmed JobRunner.
                If provided, uses these instead of creating new ones.
        """
    ...
    def _warn_if_closed(self, method_name: str) -> bool:
        ...
        if self._closed:
            _log.warning(f"BackpressureController.{method_name}() called after close()")
            return True
        return False
    ...
    @property
    def capacity_event(self) -> Optional[multiprocessing.synchronize.Event]:
    ...
    def get_pending_jobs(self) -> int:
        ...
        # Capture reference to avoid TOCTOU race with close()
    ...
    def close(self) -> None:
        ...
        if capacity_event is not None:
            try:
                ...
                _log.debug(f"Could not set capacity event during close (may be invalid): {e}")

        # Clear local references to allow GC
        self._pending_jobs = None  # type: ignore[assignment]
        self._pending_bytes = None  # type: ignore[assignment]
        self._capacity_event = None  # type: ignore[assignment]
