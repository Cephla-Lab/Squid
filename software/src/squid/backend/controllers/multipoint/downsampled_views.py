...
- Per-well images at multiple resolutions (e.g., 5, 10, 20 um)
...
# Import ZProjectionMode from _def - this is added in this PR
...
def stitch_tiles(
    ...
    if len(tiles[0][0].shape) == 3:
        canvas = np.zeros(
            (canvas_height, canvas_width, tiles[0][0].shape[2]), dtype=dtype
        )
    else:
...
def parse_well_id(well_id: str) -> Tuple[int, int]:
    ...
    if not letter_part:
        raise ValueError(
            f"Well ID '{well_id}' missing row letter(s) (e.g., 'A', 'B', 'AA')"
        )
    if not number_part:
        raise ValueError(
            f"Well ID '{well_id}' missing column number (e.g., '1', '12')"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Well ID '{well_id}' has invalid column number '{number_part}'"
        )

    if col < 0:
        raise ValueError(
            f"Well ID '{well_id}' has invalid column number '{number_part}' (must be >= 1)"
        )
...
def ensure_plate_resolution_in_well_resolutions(
    ...
    Args:
        well_resolutions: List of well resolution values in um
        plate_resolution: Plate resolution value in um
...
class DownsampledViewManager:
    ...
    def __init__(
        ...
        self.channel_names = channel_names or [
            f"Channel_{i}" for i in range(num_channels)
        ]
        self.dtype = dtype
        ...
        self.plate_view = np.zeros(
            (num_channels, plate_height, plate_width), dtype=dtype
        )
        self._log.info(
    ...
    def update_well(
        self, row: int, col: int, well_images: Dict[int, np.ndarray]
    ) -> None:
        ...
        if row < 0 or row >= self.num_rows:
            self._log.warning(
                f"Well row {row} out of bounds (0-{self.num_rows - 1}), skipping update"
            )
            return
        if col < 0 or col >= self.num_cols:
            self._log.warning(
                f"Well col {col} out of bounds (0-{self.num_cols - 1}), skipping update"
            )
            return
        ...
        for ch_idx, well_image in well_images.items():
            if ch_idx >= self.num_channels:
                self._log.warning(
                    f"Channel index {ch_idx} exceeds num_channels {self.num_channels}"
                )
                continue
            ...
            self.plate_view[ch_idx, y_start:y_end, x_start:x_end] = well_image[
    ...
    def save_plate_view(self, path: str) -> None:
        ...
        self._log.info(
            f"Saved plate view to {path} with {self.num_channels} channels"
        )
...
class WellTileAccumulator:
    ...
    def __init__(
        ...
        self.channel_names = channel_names or [
            f"Channel_{i}" for i in range(total_channels)
        ]
        ...
        if self.z_projection_mode == ZProjectionMode.MIDDLE:
            ...
            if key not in self.mip_tiles:
                ...
                if current_tile.shape != tile.shape:
                    raise ValueError(
                        f"Incompatible tile shapes for MIP at {key}: "
                        f"current={current_tile.shape}, new={tile.shape}"
                    )
                np.maximum(
                    current_tile, tile, out=current_tile
