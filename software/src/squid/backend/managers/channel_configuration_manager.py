...
class ChannelConfigurationManager:
    ...
    This eliminates duplication of channel metadata across objectives.

    Attributes:
        channel_definitions: Global channel definitions (Tier 1)
        objective_settings: Per-objective channel settings (Tier 2)
        confocal_mode: Whether confocal overrides are active
        config_root: Path to profile-specific configurations
    ...
    objective_settings: Dict[str, Dict[str, ObjectiveChannelSettings]]
    confocal_mode: bool
    ...
    def remove_channel_definition(self, channel_name: str, base_config_path: Optional[Path] = None) -> List[str]:
    ...
    def _on_update_configuration_command(self, cmd: UpdateChannelConfigurationCommand) -> None:
        ...
        if not mode:
            self._log.error(f"Configuration '{cmd.config_name}' not found for objective '{cmd.objective_name}'")
            return

        # Update the fields that are provided
        if cmd.exposure_time_ms is not None:
            self.update_configuration(cmd.objective_name, mode.id, "ExposureTime", cmd.exposure_time_ms)
