...
def _write_group_metadata(path: str, ome_metadata: dict, description: str) -> None:
    ...
    try:
        ...
        _log.debug(f"Wrote {description} metadata to {zarr_json_path}")
    except OSError as e:
        _log.error(f"Failed to write {description} metadata to {path}: {e}")
...
class ZarrWriter:
    ...
    def __init__(self, config: ZarrAcquisitionConfig):
        ...
        self._fov_positions: Dict[int, Tuple[float, float, float]] = {}  # fov_idx -> (x_mm, y_mm, z_mm)
    ...
    def initialize(self) -> None:
        ...
        if self._initialized:
            _log.warning("Writer already initialized")
            return
        ...
        try:
            ...
            _log.info("Zarr v3 dataset initialized successfully")
    ...
    def _write_zarr_metadata(self) -> None:
        ...
        if self._is_ome_ngff_array_path():
            ...
            try:
                ...
                _log.debug(f"Wrote OME-NGFF group metadata to {zarr_json_path}")
            except OSError as e:
                _log.error(f"Failed to write zarr group metadata to {zarr_json_path}: {e}")
            ...
            try:
                ...
                with open(zarr_json_path, "w") as f:
                    json.dump(zarr_json, f, indent=2)
                _log.debug(f"Wrote OME-NGFF metadata to {zarr_json_path}")
            except (OSError, json.JSONDecodeError) as e:
                _log.error(f"Failed to write zarr metadata to {zarr_json_path}: {e}")
                raise RuntimeError(f"Failed to write zarr metadata: {e}") from e

    def record_fov_position(self, x_mm: float, y_mm: float, z_mm: float, fov: Optional[int] = None) -> None:
        """Record stage position for a FOV. Only records the first call per FOV index.

        Args:
            x_mm: Stage X position in millimeters
            y_mm: Stage Y position in millimeters
            z_mm: Stage Z position in millimeters
            fov: FOV index (0 for 5D single-FOV writers, FOV index for 6D)
        """
        key = fov if fov is not None else 0
        if key not in self._fov_positions:
            self._fov_positions[key] = (x_mm, y_mm, z_mm)
    ...
    def finalize(self) -> None:
        ...
        if self._finalized:
            _log.warning("Writer already finalized")
            return

        _log.info("Finalizing Zarr v3 dataset...")
        ...
        try:
            if os.path.exists(zarr_json_path):
                ...
                if "_squid" in attrs:
                    ...
                    # Write per-FOV stage positions if recorded
                    if self._fov_positions:
                        positions = []
                        for fov_idx in sorted(self._fov_positions):
                            x, y, z = self._fov_positions[fov_idx]
                            positions.append({"fov": fov_idx, "x_mm": x, "y_mm": y, "z_mm": z})
                        attrs["_squid"]["fov_positions"] = positions

                # Add OME-NGFF translation transform for 5D single-FOV datasets
                if self._config.ndim == 5 and self._fov_positions and 0 in self._fov_positions:
                    x, y, z = self._fov_positions[0]
                    ...
                    if multiscales:
                        ...
                        if datasets:
                            ...
                            transforms.append({
                                "type": "translation",
                                "translation": [0.0, 0.0, z_um, y_um, x_um],
                            })
            ...
            _log.error(f"Failed to finalize zarr metadata at {zarr_json_path}: {e}")
            # Don't raise - data is already written, just log the metadata issue

        self._finalized = True
        self._cleanup_event_loop()
        _log.info(f"Zarr v3 dataset finalized: {self._config.output_path}")

    def abort(self) -> None:
        """Abort and clean up (blocking)."""
        _log.warning("Aborting Zarr writer...")
        ...
        try:
            ...
            try:
                ...
                _log.error(f"Failed to update abort metadata at {zarr_json_path}: {e}")
        finally:
            # Always mark as finalized and cleanup resources
            self._finalized = True
            self._cleanup_event_loop()
            _log.warning(f"Zarr writer aborted: {self._config.output_path}")
    ...
    def _cleanup_event_loop(self) -> None:
        ...
        if self._loop is not None and self._owns_loop and not self._loop.is_closed():
            try:
                ...
                _log.warning(f"Error closing event loop: {e}")
        self._loop = None
