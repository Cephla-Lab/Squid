...
pip3 install napari[all] scikit-image dask_image ome_zarr aicsimageio basicpy pytest pytest-qt pytest-xvfb gitpython matplotlib pydantic_xml pyvisa hidapi filelock lxml_html_clean psutil ndv
...
ICON_PATH="$SQUID_SOFTWARE_ROOT/assets/icon/cephla_logo.svg"
cat > "$DESKTOP_FILE" << EOF
[Desktop Entry]
Name=Squid_hcs
Icon=$ICON_PATH
Exec=gnome-terminal --working-directory="$SQUID_SOFTWARE_ROOT/src" -e "/usr/bin/env python3 $SQUID_SOFTWARE_ROOT/src/main_hcs.py"
Type=Application
Terminal=true
EOF
